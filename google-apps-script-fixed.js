...
/**
 * Handle quiz data submission after verification
 */
function handleQuizDataSubmission(params) {
  try {
    console.log('Handling quiz data submission');
    
    var phone = params.phone;
    var quizAnswersJson = params.quizAnswers;
    
    if (!phone || !quizAnswersJson) {
      return createFormResponse(false, 'Phone and quiz answers required');
    }
    ...
    if (!leadJson) {
      ...
      return createFormResponse(false, 'No lead data found');
    }
    ...
    return createFormResponse(false, 'Failed to save lead data');
  }
}
