...
function addLeadToSheet(leadData) {
  try {
    ...
    if (!leadSheet) {
      ...
      // Add section dividers with background colors
      // Lead Info (columns A-C)
      leadSheet.getRange(1, 1, 1, 3).setBackground('#2E5090');
      
      // Customer Info (columns D-G) 
      leadSheet.getRange(1, 4, 1, 4).setBackground('#1B365D');
      
      // Verification (columns H-J)
      leadSheet.getRange(1, 8, 1, 3).setBackground('#2E5090');
      
      // Quiz Answers (columns K-P)
      leadSheet.getRange(1, 11, 1, 6).setBackground('#4A6FA5');
      
      // Set column widths for better readability
      leadSheet.setColumnWidth(1, 120); // Lead ID
      leadSheet.setColumnWidth(2, 100); // Date
      leadSheet.setColumnWidth(3, 100); // Time
      leadSheet.setColumnWidth(4, 120); // First Name
      leadSheet.setColumnWidth(5, 120); // Last Name
      leadSheet.setColumnWidth(6, 200); // Email
      leadSheet.setColumnWidth(7, 140); // Phone
      leadSheet.setColumnWidth(8, 120); // Verification Code
      leadSheet.setColumnWidth(9, 120); // Status
      leadSheet.setColumnWidth(10, 150); // Verified Date
      leadSheet.setColumnWidth(11, 150); // Age Range
      leadSheet.setColumnWidth(12, 150); // Tobacco
      leadSheet.setColumnWidth(13, 180); // Coverage Amount
      leadSheet.setColumnWidth(14, 200); // Insurance Goal
      leadSheet.setColumnWidth(15, 150); // Health Status
      leadSheet.setColumnWidth(16, 180); // Coverage Timing
...
function testAddLeadToSheet() {
  ...
  console.log('🧪 Testing lead addition to sheet with enhanced headers...');
...
/**
 * Function to recreate the sheet with new headers (deletes existing data!)
 */
function recreateLeadInfoSheet() {
  try {
    var spreadsheet = SpreadsheetApp.openById(SPREADSHEET_ID);
    var existingSheet = spreadsheet.getSheetByName(LEADS_SHEET_NAME);
    
    // Delete existing sheet if it exists
    if (existingSheet) {
      spreadsheet.deleteSheet(existingSheet);
      console.log('🗑️ Deleted existing Lead Info sheet');
    }
    
    // The addLeadToSheet function will create the new sheet with enhanced headers
    // when called next time
    console.log('✅ Ready to create new sheet with enhanced headers');
    return 'Sheet deleted. New sheet will be created with enhanced headers on next lead submission.';
