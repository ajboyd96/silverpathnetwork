// Texas Final Expense Eligibility Quiz - 20 Questions
const assessmentData = [
    ...
    {
        question: "Do you currently live in Texas?",
    ...
    {
        question: "What's your zip code in Texas?",
        image: "silverpath website photos/image2.png",
        isTextInput: true,
        placeholder: "Enter your Texas zip code (e.g., 75201)"
    }
];

let currentQuestionIndex = 0;
let assessmentAnswers = {};
const quizId = 'texas-final-expense-quiz';
...
function updateNavigationButtons() {
    ...
    nextBtn.textContent = currentQuestionIndex === assessmentData.length - 1 ? 'Get My Texas Quote →' : 'Next →';
...
function calculateTexasPremiumEstimate() {
    let basePremium = 40; // Base premium for Texas (moderate cost of living)
...
function showLeadForm() {
    ...
    const estimatedPremium = calculateTexasPremiumEstimate();
...
function showResults(estimatedPremium) {
    ...
    document.getElementById('resultsContainer').innerHTML = `
        ...
        <div class="results-image">
            <img src="silverpath website photos/happycouplewhite.png" alt="Happy couple celebrating their Texas coverage" style="max-width: 300px; border-radius: 15px;">
        </div>
        
        <p style="font-size: 20px; color: #666; margin-bottom: 30px;">
            You qualify for Texas final expense coverage!
        </p>
        ...
        <div class="premium-estimate" style="background: #f8f9fa; padding: 30px; border-radius: 15px; margin: 30px 0; text-align: center;">
            ...
            <p style="color: #666; margin: 0;">Based on your Texas assessment</p>
        </div>
        ...
        <div style="margin-top: 30px; padding: 20px; background: #fff3cd; border-radius: 10px; border-left: 4px solid #ffc107;">
            <p style="margin: 0; color: #856404; font-size: 14px;">
                <strong>Next Step:</strong> Complete your contact information to receive your personalized Texas quote from a licensed insurance professional.
            </p>
        </div>
    `;
...
function sendVerificationCode() {
    ...
    const fields = {
        ...
        quizId: 'texas-final-expense-quiz'
    };
    ...
    console.log('🔔 CALLING TEXAS ASSESSMENT SCRIPT via POST:', form.action);
    console.log('📊 Texas Quiz triple notification system activated (Google Sheets + Telegram + Email)');
...
function showVerificationPage() {
    console.log('=== SHOWING TEXAS VERIFICATION PAGE ===');
...
function verifyCode() {
    ...
    console.log('=== TEXAS VERIFICATION DEBUG ===');
    ...
    setTimeout(() => {
        if (enteredCode === currentVerificationCode) {
            console.log('✅ TEXAS VERIFICATION SUCCESSFUL - Codes match!');
            ...
            logTexasVerificationSuccess();
        } else {
            console.log('❌ TEXAS VERIFICATION FAILED - Codes do not match');
...
function resendVerificationCode() {
    ...
    .then(() => {
        console.log('✅ Texas Quiz resend request sent to Google Apps Script');
        showMessage('New verification code sent! Check Google Sheets, Telegram, and Email.', 'success');
        document.getElementById('smsCode').value = '';
        document.getElementById('smsCode').focus();
    })
    .catch(error => {
        console.log('⚠️ Texas Quiz resend error:', error);
...
// Log successful verification and send Texas quiz data to Google Sheets
function logTexasVerificationSuccess() {
    if (!currentVerificationData) {
        console.error('❌ No current verification data available');
        return;
    }
    
    console.log('📊 Logging Texas verification success and quiz data to Google Sheets');
    ...
    // Prepare Texas quiz answers in a structured format (20 questions)
    console.log('📝 Raw Texas assessment answers:', assessmentAnswers);
    
    const texasQuizData = {};
    for (let i = 0; i < 20; i++) {
        texasQuizData[`q${i + 1}`] = (assessmentAnswers[i] && assessmentAnswers[i].answer) ? assessmentAnswers[i].answer : 'No answer';
    }
    
    console.log('Texas quiz answers being sent:', texasQuizData);
    
    // Create form to send Texas quiz data to Google Apps Script
    const iframe = document.createElement('iframe');
    iframe.name = 'hidden-texas-quiz-submit';
    iframe.style.display = 'none';
    document.body.appendChild(iframe);
    ...
    form.method = 'POST';
    form.action = 'https://script.google.com/macros/s/AKfycbwAJNok2I966aEJ_ec01HuTLaP42mQtIZtHkrpuDz170b5U-wCfgl3SklA1ttLC2ff0/exec';
    form.target = 'hidden-texas-quiz-submit';
    form.style.display = 'none';
    ...
    const fields = {
        action: 'submit_lead',
        phone: currentVerificationData.phone,
        quizAnswers: JSON.stringify(texasQuizData),
        firstName: currentVerificationData.firstName,
        lastName: currentVerificationData.lastName,
        email: currentVerificationData.email,
    ...
    console.log('✅ Texas quiz data submitted to Google Sheets');
}
